...
import { Bell, ChevronDown, Zap, UserPlus, AlertTriangle, ShieldCheck, DollarSign, Flag, Plus, Menu } from 'lucide-react';
...
export default function AdminTopNavbar({ onMenuClick }: AdminTopNavbarProps) {
        ...
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          {onMenuClick && (
            <button
              type="button"
              onClick={onMenuClick}
              className="p-2 -ml-1 rounded-lg hover:bg-slate-100 transition-colors lg:hidden flex-shrink-0"
          ...
          <div className="min-w-0">
            <p className="text-xs text-slate-400 font-medium uppercase tracking-wide hidden sm:block">Admin Dashboard</p>
            <h1 className="text-sm sm:text-base font-bold text-slate-800 leading-tight truncate">{pageTitle}</h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className="flex items-center gap-2 bg-violet-600 hover:bg-violet-700 text-white text-sm font-medium px-2.5 sm:px-3 py-1.5 rounded-lg transition-colors"
            >
              <Zap size={14} />
            ...
            {showQuickActions && (
              <div className="absolute right-0 top-full mt-2 w-56 max-w-[calc(100vw-2rem)] bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden">
